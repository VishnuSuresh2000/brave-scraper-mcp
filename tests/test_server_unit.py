...
async def test_execute_tool_browser_navigate(server):
    ...
    server.browser_manager.page.goto.assert_called_once_with("https://example.com", wait_until="load")
...
@pytest.mark.asyncio
async def test_execute_tool_brave_search(server):
...
@pytest.mark.asyncio
async def test_execute_tool_brave_extract(server):
    """Test brave_extract routing."""
    with patch('src.server.BraveSearchTools') as MockTools:
        mock_instance = MockTools.return_value
        mock_instance.extract = AsyncMock(return_value=MagicMock(
